...
const PAYMENT_METHODS: { key: PaymentMethod; label: string; icon: React.ReactNode }[] = [
  { key: 'efectivo', label: 'Efectivo', icon: <Banknote className="h-6 w-6" /> },
  { key: 'tarjeta', label: 'Tarjeta', icon: <CreditCard className="h-6 w-6" /> },
  { key: 'nequi', label: 'Nequi / Transferencia', icon: <Smartphone className="h-6 w-6" /> },
];
...
export function PaymentCalculator({ order, open, onClose, onPaymentComplete }: PaymentCalculatorProps) {
  ...
  const change = useMemo(() => Math.max(0, receivedNum - order.total), [receivedNum, order.total]);
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="rounded-xl bg-muted/50 p-3 space-y-1.5">
                ...
                <div className="border-t pt-2 mt-2 flex justify-between items-center">
                  ...
                  <span className="font-display text-2xl font-bold text-primary">{formatPrice(order.total)}</span>
                </div>
              </div>
        ...
        {step === 'amount' && method && (
          <div className="animate-slide-in">
            <div className="p-4 pb-2 flex items-center gap-3">
              <Button variant="ghost" size="icon" onClick={() => { setStep('method'); setReceived(''); }}>
            ...
            <div className="px-4 space-y-3">
              ...
              <div className="rounded-xl bg-muted/50 p-4 text-center space-y-1">
                <p className="text-xs text-muted-foreground uppercase tracking-wider">Monto recibido</p>
                ...
                {method === 'efectivo' && receivedNum > 0 && (
                  <div className="pt-2 border-t mt-2">
                    ...
                    <p className={`font-display text-2xl font-bold ${canConfirm ? 'text-green-500' : 'text-destructive'}`}>
                      {canConfirm ? formatPrice(change) : `Falta ${formatPrice(order.total - receivedNum)}`}
                    </p>
                  </div>
                )}
              </div>
              ...
              {method === 'efectivo' && (
                <>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button variant="outline" size="sm" onClick={handleExact} className="text-xs">
                      Exacto
                    </Button>
                    {QUICK_AMOUNTS.map(a => (
                      <Button key={a} variant="secondary" size="sm" onClick={() => handleQuickAmount(a)} className="text-xs">
                        +{formatPrice(a)}
                      </Button>
                    ))}
                  </div>
            ...
            <div className="p-4">
              ...
              >
                ...
                {canConfirm ? 'Confirmar Pago y Enviar a Cocina' : 'Monto insuficiente'}
              </Button>
            </div>
          </div>
        )}

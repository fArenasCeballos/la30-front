    ...
    queryFn: async () => {
      try {
        ...
        let query = supabase
          .from("orders")
          .select(
            "*, order_items(*, products(id, name, siigo_code, categories(name))), payments(id, method, amount_total, amount_efectivo, amount_tarjeta, amount_nequi), siigo_invoices(id, status, error_message), profiles(id, name)",
          )

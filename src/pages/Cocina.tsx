    ...
    <div className="min-h-screen bg-foreground text-background p-4 lg:p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="ml-auto flex gap-4 text-sm">
          ...
          <span className="flex items-center gap-1 text-preparing">
            <ChefHat className="h-4 w-4" /> {enPreparacion.length} en
            preparación
          </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {confirmados.map((order) => (
          <div
            key={order.id}
            className="rounded-xl border border-border/20 bg-secondary/10 p-4 animate-slide-in"
          >
        ...
        {enPreparacion.map((order) => (
          <div
            key={order.id}
            className="rounded-xl border-2 border-preparing bg-preparing/10 p-4 animate-pulse-glow"
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-2 mb-4">
              {order.items.map((item) => (
                <div key={item.id} className="flex gap-2 text-sm">
                  <span className="font-bold text-preparing">
                    {item.quantity}x
                  </span>
                  <span>{item.product.name}</span>
                  {item.notes && (
